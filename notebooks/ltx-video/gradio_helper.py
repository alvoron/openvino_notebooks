...
def make_demo(pipe):
    def generate(prompt, width, height, num_frames, num_inference_steps, seed):
        frame_rate = 25
        generate_kwargs = {
            "width": int(width),
            "height": int(height),
            ...
            # Work around a current LTX crash in the CFG + negative-prompt path.
            "guidance_scale": 1.0,
        }

        output = pipe.generate(prompt, **generate_kwargs)
